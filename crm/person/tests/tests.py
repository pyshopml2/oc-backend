...
class PersonTestCase(PersonBaseTestCase):
	...
	def test_person_detail_get(self):
		...
		self.assertEqual(person['date_of_birth'], self.person.date_of_birth.strftime(DATE))
		self.assertEqual(person['phone_number'], self.person.phone_number)
		self.assertEqual(person['extra_phone_number'], self.person.extra_phone_number)
