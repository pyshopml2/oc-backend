...
class DocumentTestCase(DocumentBaseTestCase):
	...
	def test_document_detail_get(self):
		response = self.client.get(reverse('document:document-detail', kwargs={'pk': self.document.pk}))
		...
		self.assertEqual(document.catalog_documents.pk, self.document.catalog_documents.pk)
...
class CatalogDocumentTestCase(DocumentBaseTestCase):
	...
	def test_catalog_documents_detail_get(self):
		response = self.client.get(reverse('document:catalog-detail', kwargs={'pk': self.catalog_documents.pk}))
		...
		self.assertEqual(catalog_documents.description, self.catalog_documents.description)
	...
	def test_catalog_documents_detail_post(self):
		...
		self.assertTrue(CatalogDocuments.objects.filter(pk=catalog_id).exists())
